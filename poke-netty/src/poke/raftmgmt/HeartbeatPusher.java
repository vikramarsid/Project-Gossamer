...
public class HeartbeatPusher extends Thread {
	...
	private int sConnectRate = 4000; // msec
	...
	public void connectToThisNode(int iamNode, HeartbeatData node) {
		...
		//logger.info("Added "+ iamNode+" to monitors with mgmt port: "+ node.getMgmtport());
	...
	@Override
	public void run() {
		//logger.info("*****HeartbeatPusher-->run()*****");
		if (monitors.size() == 0) {
		//	logger.info("HB connection monitor not started, no connections to establish");
			//logger.info("*****HeartbeatPusher-->run()*****monitor.size()=0");
			return;
		} else
			{
//				logger.info("*****HeartbeatPusher-->run()***** monitor.size()>0");
//				logger.info("HB connection monitor starting, node has " + monitors.size() + " connections");
		...
		//the current thread depends on monitor's size and checks for HB's connection

		while (forever) {
			try {
//				logger.info("*****HeartbeatPusher-->run()***** thread sleep for "+sConnectRate+" time");
				Thread.sleep(sConnectRate);
				// try to establish connections to our nearest nodes
//				logger.info("*****HeartbeatPusher-->run()***** try to establish connections to our nearest nodes");
				for (HeartMonitor hb : monitors) {
					
//					logger.info("*****HeartbeatPusher for loop--->"+ hb.getHost()+" "+hb.getPort()+" "+hb.getNodeInfo());
					
					if (!hb.isConnected()) {
//						logger.info("*****HeartbeatPusher-->run()***** hb.isConnected(): "+hb.isConnected());
						try {
				...
				logger.error("Unexpected HB connector failure");
				break;
			}
		}
