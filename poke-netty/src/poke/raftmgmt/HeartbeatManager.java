...
import poke.raftmgmt.HeartbeatData.BeatStatus;
...
public class HeartbeatManager extends Thread {
	...
	static final int sHeartRate = 10; // msec
	...
	static ConcurrentHashMap<Channel, HeartbeatData> outgoingHB = new ConcurrentHashMap<Channel, HeartbeatData>();
	static ConcurrentHashMap<Integer, HeartbeatData> incomingHB = new ConcurrentHashMap<Integer, HeartbeatData>();

	public static ConcurrentHashMap<Integer, HeartbeatData> liveNodes = new ConcurrentHashMap<Integer, HeartbeatData>();
	...
	public static HeartbeatManager initManager(ServerConf conf) {
		...
		try{
			int t1=conf.getNodeId();
			int t2=(int) conf.getPort();
			//logger.info("ooo: "+t1+" "+t2);
			initLineNodes(t1, "localhost", t2, 0);//9326049165
		}
		catch(Exception e)
		{
			//logger.info("Exception in HeartbeatManager constructor: "+e);
		}
		
		return instance.get();
	}
	
	public static void initLineNodes(int n, String pHost, int port, int toNode)
	{
		//logger.info("initialized liveNodes with current node");
		
		if(!liveNodes.contains(n))
			liveNodes.put(n, new HeartbeatData(n, pHost, port, 0));
		
		logger.info(n+" "+pHost+" "+port+" "+toNode);
	}
...
//use
	...
	public Management getGeneratedHB()
	{
		return generateHB();
	}
	...
	private Management generateHB() {
		...
		try
		{
			...
			b = Management.newBuilder();
			b.setHeader(mhb.build());
			b.setBeat(h.build());
		}
		catch(Exception e)
		{
			logger.info("Exception while generating msg-->");
		}
	...
	public void run() {
		...
		while (forever) {
			...
			//	logger.info("*************************Printing contents of outgoingHB********************");
				//logger.info("outgoingHB.size() " + outgoingHB.size());

//				Set<Channel> key=outgoingHB.keySet();
//
//				for(Channel ch: key)
//				{
//					System.out.println("Key: "+ch+", Value: "+outgoingHB.get(ch));
//				}
...
//				try{
//					Management mgmt=InboundMgmtWorker.getMgmt();
//					logger.info("See the election status: "+ mgmt.getElection()+", "+mgmt.getBeat()+", "+mgmt.getGraph());
//				}
//				catch(Exception e)
//				{
//					logger.error("Exception while fetching mgmt: "+e);
//				}
				
				//LeaderElection.ElectAction.WHOISTHELEADER_VALUE
				
				if (outgoingHB.size() > 0) {
					// TODO verify known node's status
					//logger.info("outgoingHB.size() > 0");
					// send my status (heartbeatMgr)
					GeneratedMessage msg = null;

					//check if the node wants to send normal heartbeats to other nodes or only to candidate node
					//if()
					//{
						for (HeartbeatData hd : outgoingHB.values()) {
							...
							try {
								if (logger.isDebugEnabled())
									//logger.debug("Sending HB to " + hd.getNodeId());
...
//										+ " at " + hd.getHost(), e);
					...
					//}
					//else
						//send only to candidate node as a reply to it's request
					//{
						/*
						for (HeartbeatData hd : outgoingHB.values()) {
							// if failed sends exceed threshold, stop sending
				...
				*/
				...
				logger.error("Unexpected management communication failure", e);
				break;
			}
		}
