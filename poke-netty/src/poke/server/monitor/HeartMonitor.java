		...
		if (channel == null) {
			try {
				...
				if (logger.isDebugEnabled())
					logger.debug("HeartMonitor: failed to initialize the heartbeat connection");
...
//		logger.info("*****Called HeartMonitor.startHeartbeat()***** the join will initiate the other node's heartbeatMgr to reply to this node's (caller) listeners.");
		boolean rtn = false;
		try {
			Channel ch = connect();
			if (!ch.isWritable()) {
			...
			logger.info("HeartMonitor connected to " + toNodeId);
			...
			if(toNodeId == ElectionManager.getLeader()){
				logger.info("------------------------------->the leader is dead.");
				ElectionManager.setFirstTime(2);
				ElectionManager.canVote = true;
				ElectionManager.resetLeader();
			}
		}
